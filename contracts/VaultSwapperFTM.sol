...
contract VaultSwapperFtm is Initializable {
    ...
    */
    ...
    function metapool_swap(
        ...
        address underlying_pool = underlying;
        address target_pool = target;
    ...
    function _get_n_coins(address pool) internal view returns (uint256) {
        uint256 num = registry.get_n_coins(pool);
        if (num != 0) return num;
